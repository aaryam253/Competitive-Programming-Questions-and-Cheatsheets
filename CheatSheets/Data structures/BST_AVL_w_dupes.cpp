struct BSTVertex {
    ...
    BSTVertex* right;
    list<int> keys;
    // We can use this to store satellite data
    int height;
};
...
protected:
    ...
    BSTVertex* insert(BSTVertex* T, int v) {
        ...
        else if (*T->keys.begin() < v) {                                // search to the right
            T->right = insert(T->right, v);
            T->right->parent = T;
        }
        else if (*T->keys.begin() > v) {                                                 // search to the left
    ...
    int successor(BSTVertex* T) {
        if (T->right != NULL)                      // this subtree has right subtree
            ...
            return par == NULL ? -1 : *par->keys.begin();          // this is the successor of T
        }
    }
    ...
    // This will still remove the entire node, similar to erase
    BSTVertex* remove(BSTVertex* T, int v) {
        if (T == NULL)  return T;              // cannot find the item to be deleted
        
        if (*T->keys.begin() == v) {                         // this is the node to be deleted
            if (T->left == NULL && T->right == NULL)                 // this is a leaf
                ...
                *T->keys.begin() = successorV;        // replace this key with the successor's key
    ...
    BSTVertex* search_lower_bound (BSTVertex* temp, int key) {
        ...
        // Iterative
        while (temp != NULL) {
            if (key < *temp->keys.begin()) {
                prev = temp;
                temp = temp->left;
            }
            else if (*temp->keys.begin() == key) return temp; // Needed to for normal search style operation
            else {
                temp = temp->right;
            }
        }
...
public:
    ...
    int search(int v) {
        ...
        return res == NULL ? -1 : *res->keys.begin();
    }
    ...
    int lower_bound(int k) {
        return *search_lower_bound(root, k)->keys.begin();
    }
    
    int upper_bound (int k) {
        BSTVertex * temp = search_lower_bound(root, k);
        if (*temp->keys.begin() != k) return *temp->keys.begin();
    ...
    int select (int rank) {
        return *select(root, rank)->keys.begin();
    }
    
    int count (int v) {
        BSTVertex* res = search(root, v);
        return res == NULL ? -1 : res->keys.size();
    }
...
private:
    ...
    BSTVertex* insert(BSTVertex* T, int v) {       // override insert in BST class
        if (T == NULL) {                                 // insertion point is found
            ...
            // Duplicate found, inserting here
            T->keys.push_back(v);
        }
